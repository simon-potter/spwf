...
|---|---|---|
| `/workflow-core:approve-plan` Security lens | Pre-build | Flags which tasks touch security-sensitive surfaces |
| `/workflow-core:pr-create` pre-flight | Ship time | Runs gitleaks / semgrep / dep-audit automatically |
...
- For secret scanning: install gitleaks pre-commit hook (see `/workflow-core:pr-create` guidance)
```
