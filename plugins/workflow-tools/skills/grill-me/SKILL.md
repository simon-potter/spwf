...
name: grill-me
description: Gate — Challenge a plan, ideation file, or design relentlessly. Accepts a file path as $ARGUMENTS (defaults to the most recent file in todo/ if omitted). Reads the file first, then interviews until all open questions are resolved. Use before task-to-spec to ensure the idea is ready to formalise.
...
```
Grill-me complete. All questions resolved.
...
Recommended next step: /workflow-core:task-to-spec todo/{slug}.md
```
