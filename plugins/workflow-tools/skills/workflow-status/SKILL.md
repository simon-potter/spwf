...
| Todo file at `status: challenged` | `Run /workflow-tools:challenge {file}` to progress it to spec |
| Todo file at `status: ideation` | `Run /workflow-tools:challenge {file}` to pressure-test it |
| All tasks complete, no todo files advanced | `Run /workflow-tools:retrospective` — ship is done, debrief |
| Multiple competing changes, none obvious | Ask: "Which of these would you like to focus on?" and list them |
| Nothing active, nothing in backlog | `Run /workflow-tools:new-task` to capture the next piece of work |
