...
- Editing CLAUDE.md or AGENTS.md → `/spwf:claudemd-curator`
- Agent/skill quality issues → `/spwf:agent-optimise` (or fix manually)
- Doc drift → `/spwf:doc-lint`
- Dependency vulnerabilities → `/spwf:dep-audit`
...
| Issue type | Tool |
|---|---|
| CLAUDE.md bloat, stale refs, rule drift | `/spwf:claudemd-curator` |
| Agent descriptions, model assignment, tool scope | `/spwf:agent-optimise` or edit directly |
| Doc staleness, broken doc links | `/spwf:doc-lint` |
| Dependency vulnerabilities | `/spwf:dep-audit` |
| Deep security review | `/spwf:security-scan` |
| PHP code quality | `/spwf:php-code-quality-reviewer` |
