...
- **Initiation flow.** A new objective starts in Beads (`bd q "..."` → returns `<prefix>-<hash>`; prefix is project-derived, e.g. `spwf-a3f2dd` in this repo). When the work is ready for technical specification, an OpenSpec change is scaffolded under that node and the id is recorded in the change's `proposal.md` frontmatter.
...
**`bd setup claude` interaction.** Skip entirely. SPWF talks to Beads via raw CLI (`bd q`, `bd show`, `bd comment`, `bd close` for the tracker layer; `bd next`, `bd done`, `bd remember`, `bd export` for the future build-loop layer). The Beadsify spec includes a prerequisite note: "do not run `bd setup claude` — SPWF provides its own Claude Code integration." Same goes for plain `bd init` (discovered during build phase 2.1 to install the same Claude integration); the safe init is `bd init --skip-agents --skip-hooks --non-interactive`. Avoids opinionated-default conflicts and keeps the integration contract stable as Beads evolves.
...
- **Per-change opt-in via frontmatter.** The build-loop hook only activates when both `.beads/` exists AND `proposal.md` frontmatter contains `beads_story_id: <prefix>-<hash>`. Changes without that frontmatter run the normal tasks.md-driven flow.
